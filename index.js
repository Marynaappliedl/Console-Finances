...
// Calculate the differences and find the greatest gain and loss
for (let i = 1; i < totalMonths; i++) {
  let difference = finances[i][1] - finances[i - 1][1];
  differences.push(difference);
  profitGain = Math.max(profitGain, difference);
  profitLoss = Math.min(profitLoss, difference);

  if (difference === profitGain) {
    profitGainMonth = finances[i][0];
  }
  if (difference === profitLoss) {
    profitLossMonth = finances[i][0];
  }
}
